...
class Third_01_TJFX_test(unittest.TestCase):
    """车辆安装数"""
    ...
    def test_01_zone(self):
        ...
        for i in range(1, 3):  # 文档中参数序号
            self.index.F5()
            self.index.homepagemenu_click(TJFX)
            self.index.fristmenu_click(param[1]["firstmenu"])
            self.index.secondmenu_click(param[i]["secondmenu"])
            self.index.thirdmenu_click(param[i]["thirdmenu"])
            ele_menu = self.index.getheader_ele(1)  # 地区汇总
...
class Third_01_JBXX_test(unittest.TestCase):
    """基本信息"""
